...
import { columns } from "@/app/dashboard/finances/transactions/columns";
...
import { TransactionListResponse } from "@/lib/types/finances";
...
export default function TransactionsClient({
  ...
  const {
    ...
    {
      // Use the initial data from server for first render
      skip:
        page === 0 &&
        sorting.length === 0 &&
        filters.length === 0 &&
  ...
  // Use server-provided data or RTK query data
  const data =
    page === 0 && sorting.length === 0 && filters.length === 0 && !!initialData
      ? initialData
      : transactionsData;
